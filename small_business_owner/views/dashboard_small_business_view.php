...
$weeklySavingsQuery = $conn->prepare("SELECT COALESCE(SUM(s_amount), 0) as weekly_savings FROM small_business_savings WHERE B_id = ? AND YEARWEEK(s_date) = YEARWEEK(CURDATE())");
$weeklySavingsQuery->bind_param("i", $B_id);
...
$weeklyExpenseQuery = $conn->prepare("SELECT COALESCE(SUM(ex_amount), 0) as weekly_expense FROM small_business_expense WHERE B_id = ? AND YEARWEEK(ex_date) = YEARWEEK(CURDATE())");
$weeklyExpenseQuery->bind_param("i", $B_id);
...
for ($i = 5; $i >= 0; $i--) {
    ...
    $monthSavingsQuery = $conn->prepare("SELECT COALESCE(SUM(s_amount), 0) as month_savings FROM small_business_savings WHERE B_id = ? AND DATE_FORMAT(s_date, '%Y-%m') = ?");
    $monthSavingsQuery->bind_param("is", $B_id, $month);
    $monthSavingsQuery->execute();
    $monthSavingsResult = $monthSavingsQuery->get_result();
    $monthSavingsData = $monthSavingsResult->fetch_assoc();
    
    
    $monthExpenseQuery = $conn->prepare("SELECT COALESCE(SUM(ex_amount), 0) as month_expense FROM small_business_expense WHERE B_id = ? AND DATE_FORMAT(ex_date, '%Y-%m') = ?");
    $monthExpenseQuery->bind_param("is", $B_id, $month);
